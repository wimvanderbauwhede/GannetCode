...
my $run_scons_sysc_str="scons $scons_v $scons_d $scons_gen";
...
if ($clean) {
    ...
    if ($build) {
        ...
        for my $file (@symlinked_cxx_sources) {
            if (not -l "$sysc_obj_path/$file") {
                system("ln -s $cxx_source_path/$file $sysc_obj_path/$file");
            }
        }        
        # Build C++ code 
        print "chdir $cxx_testbench_path\n";
