...
********************************************************************************
                 |
  File Name      | SC_Memory_Arbiter_dual.h
-----------------|--------------------------------------------------------------
...
//==============================================================================
//! This is the SC_Memory_Arbiter_dual_intchannel module, one for each master, accessing a dual-ported memory
    /*!
...
class SC_Memory_Arbiter_dual_intchannel :
    public sc_module  ,
...
public:
    ...
    // ---------------------------- CONSTRUCTORS ---------------------------------------
    SC_HAS_PROCESS( SC_Memory_Arbiter_dual_intchannel );
    SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T> (sc_module_name nm):
        sc_module(nm)
    {
        // debug message...
...
//*---------------------------------------------------------------------------------------------------------
//* Function Name       : SC_Memory_Arbiter_dual_intchannel :: write
...
template <typename ADDR_T, typename DATA_T>
void SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T> :: write (ADDR_T addr, DATA_T data)
{
...
//*---------------------------------------------------------------------------------------------------------
//* Function Name       : SC_Memory_Arbiter_dual_intchannel :: append
//* Object              : accepts write request, then using mutexes, accesses whichever port of slave is free
//* Input Parameters    : address and data
//* Output Parameters   : None
//*---------------------------------------------------------------------------------------------------------
...
template <typename ADDR_T, typename DATA_T>
void SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T> :: append (ADDR_T addr, DATA_T data)
{
    // if MUTEX guarding slave port 1 is available, take it, use it  for
    // writing to port 1 of slave, and unlock it. Otherwise try other port
    if (p_mutex_1->trylock()==0)
    {
        pwr_slave_1->append(addr, data);
        p_mutex_1->unlock();
    }
    else if (p_mutex_2->trylock()==0)
    {
        pwr_slave_2->append(addr, data);
        p_mutex_2->unlock();
    }
    // both mutexes taken; wait for specific time, and try again (recursive)
    else
...
//*---------------------------------------------------------------------------------------------------------
//* Function Name       : SC_Memory_Arbiter_dual_intchannel :: read
//* Object              : accepts read request, then using mutex, accesses the slave when it is free
//* Input Parameters    : read address
//* Output Parameters   : None
//*---------------------------------------------------------------------------------------------------------
template <typename ADDR_T, typename DATA_T>
DATA_T&  SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T> :: read(ADDR_T addr)
{
...
//*---------------------------------------------------------------------------------------------------------
//* Function Name       : SC_Memory_Arbiter_dual_intchannel :: is_free
//* Object              : check if address is free
//* Input Parameters    : address
//* Output Parameters   : bool (free or not)
//*---------------------------------------------------------------------------------------------------------
template <typename ADDR_T, typename DATA_T>
bool  SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T> :: is_free(ADDR_T addr)
{
    return(pwr_slave_1->is_free(addr));
}//is_free()
//*---------------------------------------------------------------------------------------------------------
//* Function Name       : SC_Memory_Arbiter_dual_intchannel :: size
...
template <typename ADDR_T, typename DATA_T>
unsigned int  SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T> :: size(ADDR_T addr)
{
...
//==============================================================================
//! This is the SC_Memory_Arbiter_dual module
    /*
...
public:
    ...
    // ---------------------------- LOCALS -------------------------------------
    // One instance each of "SC_Memory_Arbiter_dual_intchannel" for each master connected to arbiter
    SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T>  master2arb1;
    SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T>  master2arb2;
    SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T>  master2arb3;
    SC_Memory_Arbiter_dual_intchannel<ADDR_T, DATA_T>  master2arb4;
    ...
    virtual const char* kind() const
        { return "SC_Memory_Arbiter_dual"; }

public:
    // ---------------------------- CONSTRUCTOR --------------------------------
    SC_HAS_PROCESS( SC_Memory_Arbiter_dual );
    SC_Memory_Arbiter_dual( sc_module_name nm ) :
        sc_module( nm ) ,
