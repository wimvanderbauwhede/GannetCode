...
public:
    ...
    // ---------------------------- EXPORTS ------------------------------------
    sc_export<SC_Fifo_if<Packet_t> > xpwr_rxfifo;    //!< Export provided (to network) for writing to Rx FIFO of this tile's transceiver
...
//        service_core.transceiver_tx_fifo.bind   (transceiver.xpwr_txfifo); // FIXME: NEED ARBITER!
        ...
        transceiver.network_rx_fifo  .bind(network_rx_fifo); // transceiver's port for writing to network is propagated upwards
