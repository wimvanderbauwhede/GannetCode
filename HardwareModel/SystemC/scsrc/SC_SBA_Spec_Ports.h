...
public:
    ...
    // append the new value (only for Word_List!!)
    void mpush(ADDR_T addr ,DATA_T data) { (*this) -> append(addr, data); }
...
public:
    ...
    // FIXME: methods for dynamic loading (only used by ds_CONFIG)
    string lib (unsigned int configid)
    {
        return (*this)->lib(configid);
    }

    string symbol (unsigned int configid)
    {
        return (*this)->symbol(configid);
    }

    SBA::ServicePair& services (SBA::Service service_id) { return  (*this)->services(service_id);};
    SBA::DynConfigTuple& configurations (unsigned int configurationid) { return  (*this)->configurations(configurationid);};
    SBA::TimingTuple& timings (SBA::Service service_id, unsigned int opcode) { return  (*this)->timings(service_id,opcode);};
