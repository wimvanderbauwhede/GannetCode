...
--        [(library, (newservices,newserviceclasses))]
        ...
        localpath=foldl1 (++) (map (\s->s++"/") (init localpathlst))
...
findLibConfigPath libname localpath = do
    ...
    if isLocal
        then return $ localpath++"/Gannet/"++libname++".yml" 
            ...
            do
                isGlobal <- doesFileExist $ evGANNET_DIR++"/SystemConfigurations/"++libname++".yml"
                if isGlobal
                    then return $ evGANNET_DIR++"/SystemConfigurations/"++libname++".yml"
