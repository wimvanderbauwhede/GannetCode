...
t2sm (tt,octxt)
    | ((length tl)==0) = do -- recursion is finished
        let
            ...
            nletc:nouterc=
                if (cmpFQN (current ctxt) "let") || (cmpFQN (current ctxt) "lambda")  -- NEW: "current" will hold an FQN so we need to test the Service Class & method
                then (outerc ctxt)        
            ...
            nctxt
                | cmpFQN (head (callerstack ctxt)) "assign" = popAssignStack ctxt -- now this is really weird -- NEW: same here, deal with FQN
                | otherwise = ctxt
                        ...
                        case x of
                            Token (GannetTokenS _) -> do                    
                                case  lookupPQN (stringInToken x) True of 
                                    ("LET","let") -> do -- WV 26/03/2008: what about LETTC? 
                                        let
                                            (tt,ctxt2)=tct1
...
appendQSym tct skind = 
    ...
    where
        ...
        Token gtvar=var 
        -- update the context, step 1: push var on stack if it's a lexical
        nctxt 
