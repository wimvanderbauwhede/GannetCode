...
-- type inferencing should be done in the HLL compiler. 
                        ...
                        let
                            notlambda = lookupPQN ( ( \(GannetLabelS nstr)->nstr ) (lto slh))  True /= ("LAMBDA","lambda")
                            nlabel=label slh
        ...
        (nsl2,nslh2,ms2,ctxt2)
            |(newtestService nslh "assign") && ((length nsl==3)||(length nsl==4)) && (notQuote ms) = 
                let
                ...
                in
                    (nnsl,nslh,ms,nctxt)
            |(newtestService nslh "read") && (length nsl==2) =
                let
                    Symbol varsym = ms                   
                    scopes=scope ctxt
