...
my $path =
  ( exists $ENV{'GANNET_DIR'} ) ? $ENV{'GANNET_DIR'} . '/Garnet-HW/' : '.';
my $sc_href  = YAML::Syck::LoadFile("$path/ServiceConfiguration.yml"); # generated by garnet.rb --yaml
my $sba_href = YAML::Syck::LoadFile("$path/SBA.yml");
...
sub num_hw1 {
	...
	my $src           = uc( $HW1[5] );
	my $to =
	    exists( $ServiceNames{$dst} ) ? $ServiceNames{$dst}
	  : ( $dst =~ /d+/ ) ? $dst
	  :                    -1;
	my $return_to =
...
sub lookup_name {
	...
	if ( exists( $ServiceNames{$name} ) ) {
		...
		return $Aliases{$aluname}[2];

		# none of the above, check if it's a service number
	}
	elsif ( exists( $Services{$name} ) ) {
		return $name;
	}
