...
def ds_YCbCr2RGB() #t Word_List (na;Base::ServiceCore*;MemAddresses&) #s/parent/parent_ptr/
    #core
  puts "DYN CORE YCbCr2RGB" #skip
    result_list=[] #C++ Word_List result_list;
    matrix1=[]
for i in 0..255
matrix1[i]=i+(i<<8)+(i<<16)
end      
    c_MAXJSAMPLE=255 
    c_CENTERJSAMPLE=128
    c_SCALEBITS	= 16	
  ...
  x=x+1
    ...
    for i in 0..matrix1.length-1 #t uint
      ...
      b =  range_limit( y + cb_b_tab[cb] )
        ...
        puts "DYN YCbCr2RGB: #{y},#{cb},#{cr}\t#{fixp(1.40200,c_SCALEBITS)*(1-c_CENTERJSAMPLE)+c_ONE_HALF}\t#{r},#{g},#{b}" #skip                    
    end

    return result_list
end # of ds_YCbCr2RGB 

def fixp(x,scalebits)	
