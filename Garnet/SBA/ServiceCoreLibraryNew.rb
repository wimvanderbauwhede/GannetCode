...
#    def SBA_SCLib.sba_GATEWAY(sba_tile,parent) #t Result (na;Base::ServiceCore*)
...
#    end
    ...
    def SBA_SCLib.ls_ALU(sba_tile,parent) #t void (na;Base::ServiceCore*) #s/parent/parent_ptr/
        #core
#iv        
...
def SBA_SCLib.ls_FPU(sba_tile,parent) #t void (na;Base::ServiceCore*) #s/parent/parent_ptr/
    #core
...
# A non-scoping seq block would be SEQ, an non-scoping par block would be BEGIN
    def SBA_SCLib.ls_BEGIN(sba_tile,parent) #t void (na;Base::ServiceCore*)  #s/parent/parent_ptr/ 
        #core
    ...
    # --------------------------------------------------------------------------
    def SBA_SCLib.ls_IF(sba_tile,parent) #t void (na;Base::ServiceCore*; MemAddresses&)  #s/parent/parent_ptr/
        #core 
    parent.ack_ok=1
...
=end
    ...
    def SBA_SCLib.ls_S_IF(sba_tile,parent)#t void (na;Base::ServiceCore*; MemAddresses&)  #s/parent/parent_ptr/
        #core 
        operation=parent.opcode #t Word
        addresses=parent.addresses() #t MemAddresses&
        valaddress=0 #t MemAddress 
        if operation==M_S_IF_return
          puts "S_RETURN" #skip
    ...
    def SBA_SCLib.ls_Math(sba_tile,parent)#t void (na;Base::ServiceCore*; MemAddresses&)  #s/parent/parent_ptr/
        #core 
        
        operation=parent.opcode #t Word
        addresses=parent.addresses() #t MemAddresses&
#C++    Word_List result_list;
        if operation==M_Math_rand        
...
# ---------------------------------------------------------------------------------------------------------
    ...
    def SBA_SCLib.ls_LET(sba_tile,parent) #t void (na;Base::ServiceCore*; MemAddresses&)  #s/parent/parent_ptr/
        #core 
        addresses=parent.addresses() #t MemAddresses&
...
#iv
    ppservice=service #C++ int ppservice=(int)service;
    print "SEQ (#{parent.service}) CORE: #{parent.current_subtask}: (#{ppservice}<>#{M_SEQ_seq}\n" 
...
#        return [result] #C++  Word_List result_list; result_list.push_back(result); parent.result(result_list);
    ...
    # -----------------------------------------------------------------------------
    def SBA_SCLib.ls_LAMBDA(sba_tile,parent) #t void (na;Base::ServiceCore*; MemAddresses&)  #s/parent/parent_ptr/
        #core
        addresses=parent.addresses() #t MemAddresses& 
#iv
...
=end
    ...
    def SBA_SCLib.ls_APPLY(sba_tile,parent) #t void (na;Base::ServiceCore*; MemAddresses&)  #s/parent/parent_ptr/
            #core 
#        service_word=sba_tile.service_manager.subtask_list.called_as(parent.current_subtask) #t Word  
#    	service=getName(service_word) #t Name_t
        addresses=parent.addresses() #t MemAddresses&
...
#ev                
    ...
    result_symbol=mkSymbol(K_B,0,0,1,0,0,0) # WV17082008: is this meaningful?
        ...
        end # if M_APPLY_apply
    end # of ls_APPLY

# ----------------------------------------------------------------------------------------------------------------------
#     
    def SBA_SCLib.ls_IO(sba_tile,parent) #t void (na;Base::ServiceCore*)  #s/parent/parent_ptr/
        #core
        addresses=parent.addresses() #t MemAddresses&
        service_word=sba_tile.service_manager.subtask_list.called_as(parent.current_subtask) #t Word        
...
def SBA_SCLib.none(sba_tile,parent) #t void (na;Base::ServiceCore*; MemAddresses&)  #s/parent/parent_ptr/    
    #core
