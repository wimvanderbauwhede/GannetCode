 ...
 */
DataTransfer::DataTransfer(Socket sock, int fd) {
...
bool DataTransfer::sendCodeLength(uint32_t len) {
    ...
    if(!mysocket.sendInt(sockfd, len)) {
        return false;
    }
#ifdef VERBOSE
    ...
    if(mysocket.recvData(sockfd, buf, 1) < 0) {
#ifdef VERBOSE
...
int32_t DataTransfer::recvCodeLength() {
    ...
    if (!mysocket.recvInt(sockfd, &len)) {
#ifdef VERBOSE
...
int32_t DataTransfer::recvCodeData(char *buf, uint32_t len) {
    ...
    if ((recvLen = mysocket.recvData(sockfd, buf, len)) < 0) {
#ifdef VERBOSE
        cout << "Receive data error or client has been closed!\n";
#endif //VERBOSE
        return -1;
