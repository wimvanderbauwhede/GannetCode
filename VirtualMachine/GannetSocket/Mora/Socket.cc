 ...
 */
void Socket::setSendRecvTimeout(int sTime, int rTime){
...
bool Socket::setSocketOption()
{
    ...
    if (setsockopt (sock_fd, SOL_SOCKET, SO_OOBINLINE,
                        (char *)&on, sizeof (on)) !=0)
    {
        return false;
    }
...
bool Socket::clientConnect(){
    ...
    if ((connect(sock_fd, (sockaddr *)&my_addr, sizeof(my_addr))) != 0)
        return false;

    setSocketOption(sock_fd);
 ...
 * Return: boolean value
 */
bool Socket::serverAccept( int *sock_accept) {
    ...
    *sock_accept = accept(sock_fd, (sockaddr *)&cliaddr, &len);
    if(*sock_accept < 0)
    {
        perror("accept");
        return false;
 ...
 * Return: boolean value
 */
bool Socket::sendUInt32(uint32_t num) {
    ...
    if (send(sock_fd, (char *)&tmp, sizeof(uint32_t), 0) != sizeof(uint32_t)) {
#ifdef VERBOSE
        perror("sendInt");
#endif //VERBOSE
        return false;    
 ...
 * Return: boolean value
 */
bool Socket::recvUInt32( uint32_t *num) {

    uint32_t tmp = 0;
    
    if (recv(sock_fd, (char *)&tmp, sizeof(uint32_t), 0) != sizeof(uint32_t)) {
#ifdef VERBOSE
        perror("recvInt");
#endif //VERBOSE
        return false;    
 ...
 */
int Socket::sendData( const char *data, uint len) {
    ...
    while(offset != len)
    {
        if((sendLen = send(sock_fd, data + offset, len - offset, 0)) < 0) {
#ifdef VERBOSE
            perror("send data");
#endif //VERBOSE
            return -1;
 ...
 */
int Socket::recvData( char *data, uint len) {
    ...
    while(offset != len)
    {
        if((recvLen = recv(sock_fd, data + offset, len - offset, 0)) < 0) {
#ifdef VERBOSE
            perror("recv data");
#endif //VERBOSE
            return -1;
...
WV: why is fd not an attribute of Socket?
 */
bool Socket::createServer() {
...
//#endif //VERBOSE
    ...
    if (!createSocket(&sock_fd))
        return false;
        
    //bind server
    if (!serverBind(sock_fd))
        return false;
        
    //server start to listen
    if (!serverListen(sock_fd))
        return false;
 ...
 * Return: the boolean value
 */
bool Socket::createHostInterface() {
