...
module GannetPerl.Emitter (
emit
) where

import Language.Haskell.TH
import GannetPerl.GenInstances (genInstances)
...
--emitExpr (DeclE (DVarDecl e)) = emit e
...
-- MkPair {p_key:: Expr, p_value:: Expr, p_type::GPType}	deriving (Eq, Show, Typeable, Data)
{--
        ftype::GPType,
        fname::String,
        fargstypes::[FuncArg],
...
emitVarDecl e = do
		let
			...
			assign_or_label 
				| isFuncType (vd_type e) = "label"
				| isBuf (vd_type e) = "buf"
				| otherwise = "assign"
			maybe_q str
				| isFuncType (vd_type e) = str
...
emitInstDecl e = do
	...
	let			
		args
			...
			| otherwise = ""
		emitstr = emitCallStr ((id_name e)++".new") [args]
	return emitstr	
...
-- data LambdaDef = MkLambdaDef  { ld_type::GPType, ld_argstypes::[FuncArg], ld_body::[Expr] }
emitLambdaDef ld 
...
fe_iterator is PureE $ PVal $ MkVal {v_name:: String, v_type:: GPType}

PServiceCall (MkServiceCall "Range" "new" rargs GPAny)
sc_name::String, sc_op::String, sc_args::[Expr], sc_type::GPType
-}	
...
-}
emitVar v@(MkVar vname (GPObj ( MkObj [] ["FileHandle"]))) = do
	ctxt <- get
	let
		niters = 100
		current = currentscope ctxt
		scopes = scope ctxt
		n = head $ getExprsFromScope vname current scopes niters
	evstr <- emit n		
	return evstr  
...
--arrayType = GPTemplObj (MkTemplObj [] ("Array":[]) [TArgT GPAny])
--hashType = GPTemplObj (MkTemplObj [] ["Hash"] [TArgT GPAny])
emitVarByType (GPTemplObj (MkTemplObj [] ("Array":[]) [TArgT GPAny])) vstr = "(Array.get "++vstr++")"
emitVarByType (GPTemplObj (MkTemplObj [] ("Hash":[]) [TArgT GPAny])) vstr = "(Hash.get "++vstr++")"
...
maybeCtor e earhs 
	| a_type e == arrayTypeAny && isRange (a_rhs e) = emitCallStr "Array.fromRange" [earhs]
	| a_type e == arrayTypeAny = emitCallStr "Array.new" [earhs]
	| a_type e == hashTypeAny = emitCallStr "Hash.new" [earhs]
	| otherwise = earhs
...
emitDeclExpr (DVarDecl e) = emit e
emitDeclExpr _ = return $ ";<decl>"
-- instance EmitG DeclExpr where emit=emitDeclExpr

--------------------------------------------------------------------------------
emitGPType t = return $ "" -- ":"++(show t)    
instance EmitG GPType where emit = emitGPType
...
isFuncType (GPFunc _) = True
isFuncType _ = False		 

isBuf (GPTemplObj (MkTemplObj _ ["Buf"] _ )) = True
isBuf _ = False
