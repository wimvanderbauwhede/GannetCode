...
data InstDecl = MkInstDecl
    {
        id_type::GPType, -- Service
        id_name::String, -- s
        id_args::[Expr] -- args
    }
	deriving (Eq, Show, Typeable, Data)

data VarDecl = MkVarDecl {vd_name:: String, vd_type:: GPType} deriving (Eq, Show, Typeable, Data)
...
data Var = MkVar {v_name:: String, v_type:: GPType}	deriving (Eq, Show, Typeable, Data)
data Pair = MkPair {p_key:: Expr, p_value:: Expr, p_type::GPType}	deriving (Eq, Show, Typeable, Data)
data OpCall = MkOpCall 
    {
        oc_name::String, oc_args::[PureExpr], oc_type::GPType --TODO: make it Expr
    }
	deriving (Eq, Show, Typeable, Data)    
...
data InstAlloc = MkInstAlloc  {
  ia_type::GPType -- which is an Object 
...
data GPType =    
            ...
            | GPAny
            | GPList [GPType]
            | GPTypeCtc GPType Bool
	deriving (Eq, Ord, Show, Typeable, Data)

data TemplArgType = TArgT GPType | TArgN Int | TArgS String 
	deriving (Eq, Ord, Show, Typeable, Data)
...
getType   (PureE ( PNumber (NInt _)))= intType -- GPBasic (MkBasic [] (NumberT (SimpleT (MkSimpleNumType "Int")))) 
getType   (PureE ( PNumber (NFloat _)))= floatType -- GPBasic (MkBasic [] (NumberT (SimpleT (MkSimpleNumType "Float"))))
getType   (PureE ( PString s))= stringType -- GPBasic (MkBasic [] (OtherT (MkOtherType "String")))
getType   (PureE ( PPair p))=p_type p
getType   (PureE ( PInstAlloc ia))= ia_type ia
getType   e = GPAny 

scalarType t = GPTemplObj $ MkTemplObj [] ["Scalar"] [TArgT t]
scalarTypeAny = scalarType GPAny
         
arrayType t = GPTemplObj $ MkTemplObj [] ["Array"] [TArgT t]
arrayTypeAny = arrayType GPAny        
        
hashType t = GPTemplObj $ MkTemplObj [] ["Hash"] [TArgT t]
hashTypeAny = hashType GPAny        

refType t = GPTemplObj $ MkTemplObj [] ["Ref"] [TArgT t]
refTypeAny = refType GPAny
fhType = GPObj $ MkObj [] ["FileHandle"]
fileHandleType = fhType 
regexType = GPObj $ MkObj [] ["RegEx"]

numType = GPBasic (MkBasic [] (NumberT (SimpleT (MkSimpleNumType "Num"))))
intType = GPBasic (MkBasic [] (NumberT (SimpleT (MkSimpleNumType "Int"))))
floatType = GPBasic (MkBasic [] (NumberT (SimpleT (MkSimpleNumType "Float"))))
stringType = GPBasic (MkBasic [] (NumberT (SimpleT (MkSimpleNumType "String"))))
