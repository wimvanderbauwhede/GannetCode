...
module GannetPerl.State.Context ( 
	Context(..), 
	currentScope,
	enclosingScope,
	emptyContext,
...
--						scopetype::Int, -- ^ let=0,lambda=1
						scope::ScopeTable, -- ^ See "GannetPerl.State.Scope"
						isServiceDecl::Bool,
						...
						handlecount::Int, -- 0 = stdin, 1= stdout, 2= stderr
						handles::HandleMap,
						count::Integer,
...
emptyContext = MkContext 0 [0] emptyScope False "_" emptyS emptyTI False "_" [] emptyTD 1 emptyC 2 emptyH 0 emptySAM 
...
type MethodMap = Hash.Map String [GPType]
emptyM :: MethodMap
emptyM = Hash.empty

-- obsolete
type TypeInfoMap = Hash.Map String (GPType,Integer)
emptyTI :: TypeInfoMap
emptyTI = Hash.empty
...
type TypeDefMap = Hash.Map GPType (GPType,Bool) 
emptyTD :: TypeDefMap
emptyTD = Hash.empty

type ConfigInstMap = Hash.Map String (GPType,Integer) 
emptyC :: ConfigInstMap
emptyC = Hash.empty

type HandleMap = Hash.Map String Int 
emptyH :: HandleMap
emptyH = Hash.empty
